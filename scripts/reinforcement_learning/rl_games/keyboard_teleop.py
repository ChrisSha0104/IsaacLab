...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            print("Current end-effector pose:", curr_state.cpu().numpy())
            vx, vy, vz, vg, stop, reset = keyboard_teleop.get_command()

            if stop:
                print("[INFO] Stopping teleoperation.")
                break
            elif reset:
            ...
            curr_state[:,0] += vx * 0.05
            curr_state[:,1] += vy * 0.05
            curr_state[:,2] += vz * 0.05
            curr_state[:,7] = vg
            env.unwrapped.base_actions = curr_state
            print("Base end-effector pose:", curr_state.cpu().numpy())
