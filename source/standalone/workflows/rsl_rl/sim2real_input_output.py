...
def main():
    ...
    qpos_traj = torch.from_numpy(np.loadtxt("RRL/sim2real/input_output/traj2/qpos_goal.txt")).float() # without gripper pose
    qpos_traj = qpos_traj.to('cuda:0')[:, :7]
    obs_traj = torch.from_numpy(np.loadtxt("RRL/sim2real/input_output/traj2/policy_input_list.txt")).float()  # load teleop observations
    obs_traj = obs_traj.to('cuda:0')  # ensure it's on the same device as qpos_traj
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # agent stepping
            obs[:,10:20] = obs_traj[i].reshape(1, -1).to('cuda:0')[:,10:20]
            ...
            i += 1
            if i >= qpos_traj.shape[0]:
                break
            
    
    # ./isaaclab.sh -p source/standalone/workflows/rsl_rl/sim2real_input_output.py --task XArm-Residual-Cube-State-v2 --num_envs 1 --load_run state-based-deployable-model-1500 --checkpoint model_1500
