...
class XArmCubeResidualCamLocalBinaryV5EnvCfg(DirectRLEnvCfg):
    ...
    # debug options
    print_all_intermediate_value = True
    debug_ee = False
    ...
    def _pre_physics_step(self, ee_residual: torch.Tensor):
        ...
        if self.cfg.store_traj_data:
            ...
            if len(self.sim_ee_goal_with_res) == 400:
                save_to_txt(self.sim_ee_goal_with_res, "RRL/sim2real/sim_traj1_data/ee_with_residual/sim_ee_goal_with_res.txt")
                print("ee goal with residual saved")

        # if self.time_step_per_env == 399:
        #     actions = np.concatenate([t.detach().cpu().numpy() for t in self.sim_ee_goal_with_res], axis=0)  # shape: (100, 7)
    ...
    def _get_observations(self) -> dict:
        ...
        if self.cfg.store_traj_data:
            ...
            if len(self.sim_teleop_base_fr) == 401: # includes initial get_obs() call
                save_to_txt(self.sim_teleop_base_fr[1:], "RRL/sim2real/sim_traj1_data/base_traj/teleop_comm_b.txt")
                print("teleop comm base fr saved")
                save_to_txt(self.sim_robot_ee_b[1:], "RRL/sim2real/sim_traj1_data/robot_obs/sim_robot_ee_b.txt")
