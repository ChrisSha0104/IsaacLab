...
@configclass
class XArmCubeResidualCamLocalBinaryNewEnvCfg(DirectRLEnvCfg):
    # env 
    ...
    observation_space = 14 + 8 + 120*120 # last 3 wp + last 3 expert wp
    state_space = 3*8 + 4*8 + 120*120 + 7 + 1
    rerender_on_reset = False

    # simulation
    sim: SimulationCfg = SimulationCfg(
        dt=1 / 120, # TODO: change (1/Hz = dt * decimation)
    ...
    ee_dist_reward_scale = 0.4

    # --- auxiliary ---
    residual_penalty_scale = -5.0
    action_penalty_scale = -0.3
    velocity_penalty_scale = -0.25
    gripper_height_scale = -10.0

class XArmCubeResidualCamLocalBinaryNewEnv(DirectRLEnv):
    ...
    cfg: XArmCubeResidualCamLocalBinaryNewEnvCfg

    def __init__(self, cfg: XArmCubeResidualCamLocalBinaryNewEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_observations(self) -> dict:
        ...
        # save_tensor_as_txt(rotated_depth, "depth_clean.txt")
        ...
        # print("teleop comm")
        # format_tensor(self.teleop_comm_obs+self.init_ee)

        depth_noised = simulate_depth_noise(depth_clean)

        # depth_noised = normalize_depth_01(depth_noised)
        depth_noised = clamp_depth_01(depth_noised)
        # save_tensor_as_txt(depth_noised.reshape(-1,1,120,120), "depth.txt")
        ...
        # var: self.robot_state_hist = []
        # if len(hist) < 30 append
        # if len(hist) > 30 remove first and append
    ...
    def _compute_rewards(self):
        ...
        cube_position_b, cube_quaternion_b = subtract_frame_transforms(
            curr_root_pose_w[:, 0:3], curr_root_pose_w[:, 3:7], self._cube.data.body_link_state_w[:,0,:3], self._cube.data.body_link_state_w[:,0,3:7]
        )
        ...
        # print("completion", self.extras["log"]["completion"])
        # print("ee dist", self.extras["log"]["ee_dist_reward"])
