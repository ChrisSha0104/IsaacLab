...
class XArmCubeResidualCamLocalBinaryV3EnvCfg(DirectRLEnvCfg):
    ...
    ee_rate_scale = -0.3
    velocity_penalty_scale = -0.04
    jerk_penalty_scale = -0.1
...
class XArmCubeResidualCamLocalBinaryV3Env(DirectRLEnv):
    ...
    def _pre_physics_step(self, ee_residual: torch.Tensor):
        ...
        if self.cfg.show_demo:
            ...
            self.ee_goal = self.training_demo_traj[self.env_idx, self.demo_idx, self.time_step_per_env, :] + self.ee_residual
        ...
        #     plt.tight_layout()
        #     plt.suptitle('Actions over Time w/ Low-pass Filter lambda=0.7', fontsize=16)
        #     plt.show()
