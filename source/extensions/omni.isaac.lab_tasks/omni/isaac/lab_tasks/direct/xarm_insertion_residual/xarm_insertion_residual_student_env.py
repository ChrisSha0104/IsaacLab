...
class XArmInsertionResidualStudentEnvCfg(DirectRLEnvCfg):
    ...
    storing_path = "RRL/tasks/insertion/sim2real/traj1"
...
class XArmInsertionResidualStudentEnv(DirectRLEnv):
    ...
    cfg: XArmInsertionResidualStudentEnvCfg

    def __init__(self, cfg: XArmInsertionResidualStudentEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _pre_physics_step(self, noutput: torch.Tensor):
        ...
        if self.cfg.enable_residual:
            nbase = self.fingertip_normalizer.normalize(self.teleop_fingertip_10D.clone())
            n_fingertip_goal_10D = nbase + self.cfg.alpha * self.n_residual_10D.clone()
            self.fingertip_goal_10D = self.fingertip_normalizer.denormalize(n_fingertip_goal_10D.clone()) 
        else: 
    ...
    def _get_observations(self) -> dict:   
        ...
        num_base_obs = 10 * (1+self.cfg.num_samples)
        ...
        normalized_state_obs = self.state_obs_normalizer.normalize(state_obs) # TODO: clean this up !!!!
        normalized_student_obs = torch.cat((normalized_state_obs, normalized_depth), dim=-1)
        ...
        teacher_obs = torch.cat(
            (
                ...
                self.intended_goal_pose, #TODO: normalize
            ),
            dim=-1,
        )
    ...
    def _print_training_info(self):
        ...
        print(f"Policy type     : {'Distillation'}")
        ...
        print(f"Visualize Depth : {'Yes' if self.cfg.show_camera else 'No'}\n")
    ...
    def _update_training_traj(self, env_ids, add_noise=True):
        ...
        self.intended_goal_pose = self.detect_goal_from_gripper_release_9d(self.training_demo_traj[env_ids, self.demo_idx[env_ids]], self.insertion_goals)
    ...
    def detect_goal_from_gripper_release_binary(
        ...
        Returns:
            ...
            - 1.0 for envs where release was closer to goals[0]
            - -1.0 for closer to goals[1]
            - 0.0 if no release happened
        """
        ...
        result = torch.zeros(num_envs, device=trajectories.device, dtype=torch.long)
        result[env_ids[has_release]] = torch.where(closer_to_goal0[has_release], 1, -1)

        if (result == 0).any():
            print("[Warning] Some environments had no gripper release — result contains 0s")
